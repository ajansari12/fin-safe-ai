...
-- Add constraints, indexes, and security functions
...
-- 8. Add constraints for data integrity (fixed syntax)
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE constraint_name = 'chk_profiles_role_valid' 
        AND table_name = 'profiles'
    ) THEN
        ALTER TABLE public.profiles ADD CONSTRAINT chk_profiles_role_valid 
