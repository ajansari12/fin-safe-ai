...
INSERT INTO kri_logs (kri_id, actual_value, measurement_date, threshold_breached, comments)
SELECT 
  kd.id,
  CASE 
    WHEN kd.kri_name = 'Operational Risk Score' THEN 82 + (RANDOM() * 10)
    WHEN kd.kri_name = 'System Downtime' THEN 99.3 + (RANDOM() * 0.5)
    ELSE 5 + (RANDOM() * 2)
  END,
...
WHERE kd.org_id = 'ce1b0f79-5c5a-4e2d-8d15-18dbbb61d845',
generate_series(1, 7);
