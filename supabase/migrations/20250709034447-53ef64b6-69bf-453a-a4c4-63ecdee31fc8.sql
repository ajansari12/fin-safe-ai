...
-- Create sample data for testing role-based access (where tables exist)
-- Only insert if tables exist to avoid errors
DO $$
BEGIN
  -- Check if risk_appetite_statements table exists
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'risk_appetite_statements') THEN
  ...
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'controls') THEN
  ...
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'incident_logs') THEN
