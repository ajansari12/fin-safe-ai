...
interface EnhancedScenarioBuilderProps {
...
const EnhancedScenarioBuilder: React.FC<EnhancedScenarioBuilderProps> = ({
  scenario,
  onSave,
  onComplete,
  onCancel
  ...
  const [builderMode, setBuilderMode] = useState<'manual' | 'template' | 'ai'>('manual');
  ...
  const handleUseTemplate = (template: ScenarioTemplate) => {
    setFormData({
      ...
      disruption_type: template.crisis_type,
  ...
  const handleAIGenerated = (generatedScenario: any) => {
    setFormData({
      title: generatedScenario.title,
      description: generatedScenario.description,
      disruption_type: generatedScenario.disruption_type,
      severity_level: generatedScenario.severity_level,
      response_plan: generatedScenario.response_plan || '',
      post_mortem: '',
      lessons_learned: generatedScenario.lessons_learned || '',
      improvements_identified: '',
      outcome: ''
    });
    setBuilderMode('manual');
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground">Select a predefined template to get started quickly</p>
          </div>
          <Button variant="outline" onClick={() => setBuilderMode('manual')}>
            Back to Builder
          </Button>
        </div>
        <ScenarioTemplateLibrary onUseTemplate={handleUseTemplate} />
      </div>
    );
  }
  ...
  if (builderMode === 'ai') {
    return (
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-lg font-semibold">AI Scenario Generator</h3>
            <p className="text-muted-foreground">Generate scenarios based on past incidents or custom requirements</p>
          </div>
          <Button variant="outline" onClick={() => setBuilderMode('manual')}>
            Back to Builder
          </Button>
        </div>
        <AIScenarioGenerator onScenarioGenerated={handleAIGenerated} />
      </div>
    );
  }
    ...
    <Card className="w-full max-w-4xl mx-auto">
      <CardHeader>
        <div className="flex justify-between items-start">
          <div>
            <CardTitle>Enhanced Scenario Test Builder</CardTitle>
            <CardDescription>
...
export default EnhancedScenarioBuilder;
