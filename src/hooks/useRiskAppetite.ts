...
export const useRiskAppetite = () => {
  ...
  const createStatement = async (statementData: {
    ...
    try {
      ...
      if (statementData.riskCategories.length > 0) {
        const { error: categoriesError } = await supabase
          .from('risk_categories')
          .insert(
            statementData.riskCategories.map(category => ({
      ...
      if (statementData.quantitativeLimits.length > 0) {
        // First get category IDs for mapping
        ...
        const { error: limitsError } = await supabase
          ...
          .insert(
            statementData.quantitativeLimits.map(limit => ({
              statement_id: statement.id,
              category_id: categoryMap[limit.category_name] || null,
              metric_name: limit.metric_name,
              limit_value: limit.limit_value,
              limit_unit: limit.limit_unit,
              warning_threshold: limit.warning_threshold,
              critical_threshold: limit.critical_threshold,
              measurement_frequency: limit.measurement_frequency,
              data_source: limit.data_source,
      ...
      if (statementData.qualitativeStatements.length > 0) {
        const { error: qualitativeError } = await supabase
          .from('qualitative_statements')
          .insert(
            statementData.qualitativeStatements.map(qual => ({
              statement_id: statement.id,
              category: qual.category,
              statement_text: qual.statement_text,
              acceptance_criteria: qual.acceptance_criteria,
              rationale: qual.rationale
            }))
          );

        if (qualitativeError) throw qualitativeError;
      }
